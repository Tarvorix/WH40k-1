...
/// Convert a DecisionSurface (from command_system) into a DecisionSurfaceView.
pub fn decision_surface_to_view(
    surface: &wh40k_command_system::DecisionSurface,
) -> DecisionSurfaceView {
    let action_views: Vec<ActionView> = surface
        .legal_commands
        .iter()
        .enumerate()
        .map(|(i, cmd)| command_to_action_view(i, cmd))
        .collect();
    ...
    DecisionSurfaceView {
        decision_type: surface.decision_type.name().to_string(),
        owner: surface.owner.0,
        actions: action_views,
        is_mandatory: surface.is_mandatory,
        num_options,
        is_forced,
    }
}

/// Convert a single Command into an ActionView (for DecisionSurface rendering).
pub fn command_to_action_view(index: usize, cmd: &Command) -> ActionView {
    let label = format!("{}", cmd);
    let command_type = cmd.category().to_string();
    let player = cmd.player().map(|p| p.0);

    let mut unit_id: Option<u32> = None;
    let mut target_id: Option<u32> = None;
    let mut position: Option<PositionView> = None;
    ...
    match cmd {
        ...
        Command::SelectUnitToMove { unit_id: uid } => {
            unit_id = Some(uid.0);
        }
        Command::SelectUnitToShoot { unit_id: uid } => {
            unit_id = Some(uid.0);
        }
        Command::SelectUnitToFight { unit_id: uid } => {
            unit_id = Some(uid.0);
        }
        Command::RemainStationary { unit_id: uid } => {
            unit_id = Some(uid.0);
        }
        Command::DeclareShootingTargets {
            ...
            if let Some(first_target) = targets.first() {
                target_id = Some(first_target.1 .0);
            }
        }
        ...
        Command::ChooseVaultswordsProfile { .. } => {
            // model_id based, no unit_id directly
        }
        Command::PileIn {
            unit_id: uid, ..
        } => {
        ...
        Command::AllocateWound {
            ...
            // Model-level, expose the model_id as unit_id (best effort)
            unit_id = Some(target_model_id.0);
        }
        Command::AssignOverwatchTarget { unit_id: uid } => {
            unit_id = Some(uid.0);
        }
        // Commands that don't have unit or position info
